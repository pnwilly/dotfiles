...
- Use a dedicated worktree root per project: `~/aiwork/worktrees/<project>/<branch-slug>/`, slug replaces `/` with `-`. Never ad-hoc siblings of the primary clone, and never under `/tmp` — it is tmpfs and reaped on a timer, so uncommitted work does not survive a reboot.
